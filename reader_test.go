...
func TestBasicDecompress(t *testing.T) {
	tt := []struct {
		msg        string
		data       []byte
		want       []byte
	...
	for _, v := range tt {
		...
		if v.wantErrStr == "" {
			if err != nil {
				t.Fatalf("error reading from bzip decoder: %s", err)
			}
			if !reflect.DeepEqual(v.want, out.Bytes()) {
				t.Fatalf("bzip2 output did not match expected")
			}
		} else {
			if err.Error() != v.wantErrStr {
				t.Fatalf("wanted err: %s, got: %s", v.wantErrStr, err.Error())
			}
		}
	}
}
...
func TestVaryingSizes(t *testing.T) {
	sizes := []int{0, 1, 1024, int(2.5 * 1024), 10 * 1024, int(10.25 * 1024), 1025 * 1025}
	for _, v := range sizes {
		t.Logf("generating bzip2 with source data of len: %d", v)
		var raw bytes.Buffer
		_, err := io.CopyN(&raw, rand.Reader, int64(v))
		if err != nil {
			t.Fatalf("error generating random data: %s", err)
		}
		var compressedGo bytes.Buffer
		var compressed bytes.Buffer
		cmWrtr := io.MultiWriter(&compressed, &compressedGo)
		cmprsr, err := NewWriter(cmWrtr)
		if err != nil {
			t.Fatalf("unable to make bzip compressor: %s", err)
		}
		_, err = io.Copy(cmprsr, &raw)
		if err != nil {
			t.Fatalf("error compressing random data: %s", err)
		}
		cmprsr.Close()
		var cDecomp bytes.Buffer
		cRdr, err := NewReader(&compressed)
		if err != nil {
			t.Fatalf("unable to make bzip decompressor: %s", err)
		}
		_, err = io.Copy(&cDecomp, cRdr)
		if err != nil {
			t.Fatalf("error decompressing with c: %s", err)
		}
		var gDecomp bytes.Buffer
		gRdr := bzip2.NewReader(&compressedGo)
		_, err = io.Copy(&gDecomp, gRdr)
		if err != nil {
			t.Fatalf("error decompressing with go: %s", err)
		}
		if !reflect.DeepEqual(gDecomp.Bytes(), cDecomp.Bytes()) {
			t.Fatalf("go decompressed and c decompressed data does not match")
		}
	}
}
...
func TestEOF(t *testing.T) {
	var raw bytes.Buffer
	_, err := io.CopyN(&raw, rand.Reader, 1024*1024)
	var compressed bytes.Buffer
	cmprsr, err := NewWriter(&compressed)
	if err != nil {
		t.Fatalf("unable to make bzip compressor: %s", err)
	}
	_, err = io.Copy(cmprsr, &raw)
	...
	cmprsr.Close()
	cSpecial := newSpecialReader(t, compressed.Bytes())
	var cDecomp bytes.Buffer
	cRdr, err := NewReader(cSpecial)
	if err != nil {
		t.Fatalf("unable to make bzip decompressor: %s", err)
	}
	_, err = io.Copy(&cDecomp, cRdr)
	...
	var gDecomp bytes.Buffer
	gRdr := bzip2.NewReader(&compressed)
	_, err = io.Copy(&gDecomp, gRdr)
	if err != nil {
		t.Fatalf("error decompressing with go: %s", err)
	}
...
func newSpecialReader(t *testing.T, b []byte) *specialReader {
